class ZeroFile {
  ...
  addPart(partNum, partData) {
    if (this.parts.length === 0) {
      return Messenger && Messenger.log(this, 'Already have all parts');
    }

    // TODO Can assume the input is valid? Can remove it?
    if (!partNum || !partData) {
      throw new Error(`ZeroFile::addPart: ${partNum}, {partData}`);
    }

...
*Djezzy* is an Algerian mobile network operator. Djezzy provides a free text-only version of Facebook: Facebook Zero (0.facebook.com) or 0FB for short.
...
So, to send a photo (or any file for that matter), first convert it to text (*base64*) and send it as a text message. On the other side, the recipient should convert it back to binary and view it.
...
- **id**: As far as I know, Facebook assigns an id (*FBID*) to each of its objects (profiles, groups, posts, messages, etc.).  
...
// In case we're in 0.facebook.com/zuck, 'id' will be '4'
```
- **My id**  
![My profile in Italian](screenshots/0FB-moi.png)  
We assume I'm already logged in. To get get my id, we to my profile page (`/profile.php`) and extract it from the "Registo de atividade" ("Activity Log") link.  
...
```js
// We can use the above information to do this:
...
// The following will be an empty string or (probably) a two-character name
...
### Conversations
![0FB Chat, group, blocked](screenshots/0FB-chat-group.png)
![0FB Chat, direct, active](screenshots/0FB-chat-direct.png)

We can see a chat page's markup like this (or at least that's what I recall):
```html
<div> <!-- HEADER -->
  <strong>Name!</strong>
  <span>greenDot?</span>
...
- Each conversation has an id (*cid*), AKA thread id (*tid*).
  ...
  - Individual conversations contain user's id and my id: `cid=ID_X:ID_Y`. My id is either ID_X or ID_Y... Having already obtained my id, to get my-not-id we now have the recipient's id.
  - We can use the individual chat id to get more information about them as shown in the **Profiles** section, using a link like `/profile.php?fbid=<THEIR_ID>&v=info`.
...
As for sending messages, we use a simple function that performs the actions a user normally does: Fill in a message and then click send (submit).
...
### ZeroWorker
A ZeroWorker (presented in the **Gathering Data** section) runs on iframes that are opened by **Master**. (Their scripts get automatically injected in 0FB iframes thanks to Chrome Extension APIs).
...
**This is the reason we have been attaching everything to the ZeroWorker namespace: To dynamically find needed functions.**
```js
...
As for how it works:
  ...
  - It sends ("posts") the job to the worker and starts listening for a response whose id is same.
  - When a response arrives, the promise is either resolved (or rejected if something goes wrong).
```js
class Master {
  // ...
...
These classes open different pages/links to do different things. For example, to get a user's info, you open their profile info page then call worker (by specifying `getProfileInfo` as the job's function) to read and send that info to you.

Messenger contains all the other parts/classes and eases communication between them. For instance, to distinguish between my id and another user's, `Profile.getTheirId(url)` needs to know my id which is stored in `Messenger.moi.id`.

As for dynamically updating the content, ZeroMessenger regularly checks 0FB, the same way a user refreshes pages every few seconds. *Really, the goal of this project was to mimic the user's actions + add photos.*

---

By this point, I have created a good enough API for working with Facebook Zero; the rest is a basic chat/instant-messaging app.
...
The bot logs in using my account and starts watching for new messages that I send to myself.
...
* **Receiving**: If I receive a file (a photo for example), the bot should:
  ...
  - And finally sends those messages to my inbox.

* **Sending**: If I want to send a file to someone:
  - I simply select something and send it, as in any messaging app.
  - ZeroMessenger reads the file and sends its textual representation to my inbox, in addition to some metadata: whom it is sent to, etc.
  - ZeroBroker collects those pieces, converts them to a binary file, then sends that file to the recipient as if it were me.
  - The Broker informs me of the progress by sending updates to my inbox, as if I were messaging myself. Weird!
...
Data is half processed by Worker and the other half by Messenger/Master.
That's confusing, only one of them should take the responsibility, preferably Messenger while Workers only gather raw data and "obey orders."

---

Facebook tries to prevent scrappers and such; the way ZeroMessenger works is similar to crawlers, which necessitates that we mimic a browser. I could use libraries to grab pages (using `axios`), parse them and extract relevant info (using `cheerio`), compose requests and send them. This solution would be independent of the browser, and works on Node, the possibilities would be limitless.

That wasn't what I did. Wanting to keep it simple and having used Google Chrome's extensions API before, I decided to use iframes and inject scripts into them. This is a bad approach since it's costy (needlessly rendering pages and loading images) and gives less control (like catching network errors and redirects and stuff)
...
- *Nothing beats learning by doing: In reality, this was a huge experimental project: VueJS, Promises and async/await, postMessage API, "parallelism", etc.*
